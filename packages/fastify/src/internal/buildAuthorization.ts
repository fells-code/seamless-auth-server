...
export function buildServiceAuthorization(req: FastifyRequest) {
...
const PROXY_TOKEN_SUBJECT = "seamless-auth-fastify-adapter";

export function buildProxyServiceAuthorization(
  opts: Pick<SeamlessAuthServerOptions, "serviceSecret" | "jwksKid">,
): string | undefined {
  // getSeamlessUser is callable standalone, without the serviceSecret the plugin
